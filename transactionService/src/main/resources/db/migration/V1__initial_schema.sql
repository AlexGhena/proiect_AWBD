...
CREATE TABLE scheduled_transactions (
    ...
    CONSTRAINT ck_scheduled_currency CHECK (currency ~ '^[A-Z]{3}$'),
...
COMMENT ON COLUMN scheduled_transactions.source_account_id IS
    'Logical BankAccount reference validated through bankingService; no cross-database foreign key.';
COMMENT ON COLUMN scheduled_transactions.destination_account_id IS
    'Logical BankAccount reference validated through bankingService; no cross-database foreign key.';
...
COMMENT ON COLUMN bank_transactions.source_account_id IS
    'Logical BankAccount reference validated through bankingService; no cross-database foreign key.';
COMMENT ON COLUMN bank_transactions.destination_account_id IS
    'Logical BankAccount reference validated through bankingService; no cross-database foreign key.';
