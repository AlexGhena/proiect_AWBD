 ...
 * call to userService, so the downstream service can validate the same user independently.
 */
class BearerTokenPropagationInterceptorTests {
    ...
    private MockClientHttpRequest outboundRequest() {
        return new MockClientHttpRequest(HttpMethod.GET, URI.create("http://user/api/users/1"));
    }
