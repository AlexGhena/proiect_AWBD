...
Tests run against the `test` Spring profile with an in-memory H2 database (`src/test/resources/application-test.yml`), so they don't need Postgres running. The `db/migration` scripts are Postgres-specific (`gen_random_uuid()`, `TIMESTAMPTZ`, the `~` regex operator, partial indexes, `ON CONFLICT`), so H2 can't run them; the test profile instead points `spring.flyway.locations` at a **parallel, dialect-ported migration set** under each service's `src/test/resources/db/h2migration/V*.sql` (same version numbers, H2 syntax: `RANDOM_UUID()`, `REGEXP_LIKE()`, `TIMESTAMP WITH TIME ZONE`; userService emulates the "one default address per profile" partial unique index with a generated marker column). Everything lands in H2's default `PUBLIC` schema — which is also why userService's test config sets `app.security.jdbc.schema: PUBLIC`. **Keep the two migration sets in lockstep**: any change to a `db/migration` script needs the matching `db/h2migration` edit (and vice versa), or `hibernate.ddl-auto: validate` will fail the test boot.
